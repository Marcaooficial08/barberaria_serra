...
const BookingItemAdm = ({ booking }: BookingItemProps) => {
  ...
  if (!booking || !booking.service || !booking.user) {
    return null; // Ou uma mensagem de erro ou fallback adequado
  }
    const [isDeleteLoading, setIsDleteLoading] = useState(false);


  const isBookingConfirmed = isFuture(booking.date);
 const handCancelClick = async () =>{
      setIsDleteLoading(true)
       try{
  ...
  return (
    ...
    <Button 
     disabled={!isBookingConfirmed || isDeleteLoading} className="w-full"    variant="destructive">
  ...
  <AlertDialogContent className="w-[90%]" >
    <AlertDialogHeader>
      ...
      <AlertDialogDescription>
      Essa ação não pode ser desfeita. Isso excluirá permanamente o seu agendamento.
      </AlertDialogDescription>
    </AlertDialogHeader>
    <AlertDialogFooter className="flex-row gap-3">
      <AlertDialogCancel className="w-full mt-0">Voltar</AlertDialogCancel>
      <AlertDialogAction disabled={isDeleteLoading} className="w-full"onClick={handCancelClick}>
